  ...
  async getAllMovements() {
    try {
      const movements = await this.stockMovementRepository.find({
        relations: ['stock', 'client', 'supplier', 'stock.product'],
      });
  
      if (!movements || movements.length === 0) {
        throw new NotFoundException('No se encontraron movimientos de stock');
      }
  
      return movements.map((movement) => ({
        id: movement.id,
        productName: movement.stock.product ? movement.stock.product.name : 'Unknown Product',
        sector: movement.stock.product ? movement.stock.product.sector : 'Unknown Sector',
        type: movement.type,
        quantity: movement.quantity,
        client: movement.client ? movement.client.name : null,
        supplier: movement.supplier ? movement.supplier.name : null,
        stockQuantity: movement.stock.actual_quantity,
      }));
    } catch (error) {
      console.error('Error al obtener los movimientos de stock:', error);
      throw new BadRequestException('Error al obtener los movimientos de stock');
    }
  }
